...
//#define _DEBUG
#define TEST_WAIT_MILI 10000 // how many miliseconds do we wait before assuming a test is hung
...
static int test3(void){
    ...
    //cout<<"after 2 thread join a41(11) and a42(12) are "<<a41<<" "<<a42<<endl;
    if (a41 != 11 || a42 != 12)
        return FAIL;
    return PASS;
}
...
static void* _thread_sem2(void* arg){
    ...
    a6 = local_var;
	cout<<"inside wrapper, a6 is now "<<a6<<endl;
    sem_post(sem);
    pthread_exit(0);
}
...
static int test6(void){
    ...
    sem_destroy(&sem);
	cout<<"value of a6 is "<<a6<<endl;
	cout<<"it should be "<<10 + (tid1*5) + (tid2*5)<<endl;
...
//static int (*test_arr[5])(void) = {&test0, &test1, &test2, &test3, &test4};
